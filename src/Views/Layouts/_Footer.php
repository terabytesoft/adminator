...
<!- FOOTER !->
<?= Html::beginTag('footer', ['class' => 'footer ' . $this->app->params['adminator.footer.css']]) ?>
	<?= Html::beginTag('div', ['class' => 'container fsz-sm']) ?>
		<?= Html::beginTag('div', ['class' => 'row']) ?>
			<?= Html::beginTag('div', ['class' => 'col-sm ta-l']) ?>
				...
				<?= Html::tag('span', '&nbsp' . $this->app->params['adminator.copyright'], ['class' => 'align-middle']) ?>
			<?= Html::endTag('div') ?>
			<?= Html::beginTag('div', ['class' => 'col-sm d-none d-lg-inline']) ?>
				<?= Html::tag('span', 'POWERED BY: ', ['class' => 'align-middle']) ?>
				<?= html::img($this->app->params['adminator.logo-yii'], ['class' => 'align-middle', 'style' => 'width:150px;']) ?>
			<?= Html::endTag('div') ?>
			<?= Html::beginTag('div', ['class' => 'col-sm ta-r d-none d-sm-inline']) ?>
				<?= Html::beginTag('span', ['class' => 'align-middle', 'style' => 'padding-right: 5px']) ?>
					<?= $this->app->t('ThemeAdminator', 'SOCIAL NETWORKS:') ?>
				<?= Html::endTag('span') ?>
				<?= Html::a(
					'',
					$this->app->params['adminator.footer.account.github'],
					[
				...
				<?= Html::a(
					'',
					$this->app->params['adminator.footer.account.facebook'],
					[
				...
				<?= Html::a(
					'',
					$this->app->params['adminator.footer.account.twitter'],
					[
				...
				<?= Html::a(
					'',
					$this->app->params['adminator.footer.account.linkedin'],
					[
